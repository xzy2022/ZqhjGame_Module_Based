# 修改时间：2026-09-18
# 修改目的：迁移队友交付的因果时序关联和相机运动补偿实现。
# 修改内容：保留原始跟踪算法，作为 FrontierPipeline 的包内模块。
...
class TemporalTracker:
    ...
    def update(self,detections,probabilities,timestamp,sequence_id,motion=None,image_shape=(768,1024)):
        if self.sequence!=sequence_id or (self.previous_t is not None and (timestamp<=self.previous_t or timestamp-self.previous_t>self.max_gap_s)):
        ...
        for i,j in assignments:
            ...
            # EMA never consumes future probabilities and resets with each sequence.
        ...
        # Predictions without a current-frame detection remain internal; no invented boxes.
        h,w=image_shape
